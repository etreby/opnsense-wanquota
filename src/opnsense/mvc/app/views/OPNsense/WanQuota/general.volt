 ...
 *
 * This printed "(dry-run)" whenever the guardrail could not act, which includes
 * enforcement simply being switched off — the default. A firewall with enforcement off
 * therefore looked like one in dry-run, and someone who had deliberately gone live
 * elsewhere read it as their setting having been reverted.
 */
function guardrailState(policy) {
    switch ((policy || {}).state) {
        case 'live': return '(enforcing)';
        case 'dry_run': return '(dry-run)';
        case 'observe_only': return '(observe only)';
        case 'off': return '(enforcement off)';
        default: return '';
    }
}
function intelligenceCards(data){const providers=data?.summary?.providers||[];return providers.map(p=>{const f=p.forecast||{},q=p.quality||{},policy=p.policy||{},movement=p.movement;const trend=movement?(movement.used_delta>=0?'▲ ':'▼ ')+gb(Math.abs(movement.used_delta))+' in 24h':'Collecting 24h comparison';return `<div class="wq-card" data-filter="${esc((p.name+' '+f.risk+' '+policy.recommended).toLowerCase())}"><div class="wq-muted">${esc(p.logical_interface)} · ${esc(q.status||'unknown')}</div><h3><a href="#" class="wq-drill" data-drill="provider" data-value='${esc(p.name)}' title="Show which devices use this WAN most">${esc(p.name)}</a></h3><div class="wq-metric wq-risk-${esc(f.risk||'on_track')}">${esc((f.risk||'unknown').replace('_',' ').toUpperCase())}</div><div>${f.exhaustion_date?'Exhaustion '+esc(f.exhaustion_date):'Projected within allowance'}</div><div><b>${esc(trend)}</b></div><div class="wq-muted">Latency ${Number(q.latency||0).toFixed(1)} ms · Loss ${Number(q.loss||0).toFixed(1)}% · Guardrail ${esc(policy.recommended||'observe')} ${esc(guardrailState(policy))}</div></div>`}).join('');}
